...
/**
 * It is able to draw an IntGrid on a LayeredSurfaceFrame using a ColorMap to render the values contained by the cell
 ...
 */
public class LayerIntGridDrawer implements LayerDrawer {
    private final int[] m;
     ...
     * @param colorRange The IColorMap used to map values to colours.
     */
    public LayerIntGridDrawer(String name, int[] matrix, int width, int height, ColorMap colorRange) {
        description = name;
        m = matrix;
        color = colorRange;
        xSize = width;
    ...
    /**
     * Create a double layer drawer using values taken from an array of integers and a given IColorMap. It allows to
    ...
    /**
     * Create a double layer drawer using values taken from an IntGrid matrix and a given IColorMap. It allows to define
     ...
     * @param transparentColor A color
     */
    public LayerIntGridDrawer(String name, IntSpace matrix, ColorMap colorRange, Color transparentColor) {
    ...
    public void paint(Graphics g, int cellLen) {
        if (transparencyColor != null) paintWithTransparency(g, cellLen);
        else paintWithoutTransparency(g, cellLen);
    }
    ...
    private void paintWithoutTransparency(Graphics g, int cellLen) {
        WritableRaster raster = img.getRaster();
        ...
        if (cellSize != cellLen) setCellLength(cellLen);
    ...
    private void paintWithTransparency(Graphics g, int cellLen) {
        WritableRaster raster = img.getRaster();
    ...
    /**
     * If a mouse listener has been defined the mouse dragging event, it is passedto it, otherwise it is returned false.
     *
