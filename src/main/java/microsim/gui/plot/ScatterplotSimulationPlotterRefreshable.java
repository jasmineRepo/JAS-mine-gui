...
/**
 * A ScatterplotSimulationPlotterRefreshable is able to trace one or more pairs of data sources between scheduled
 * simulation time-steps, creating a scatterplot chart that is updated on demand (e.g. for visualising the progress of
 * the simulation between time-steps). It is based on ScatterplotSimulationPlotter, which is itself based on JFreeChart
...
public class ScatterplotSimulationPlotterRefreshable extends JInternalFrame implements EventListener {
    ...
    /**
     * Constructor for scatterplot chart objects, featuring a toggle to hide the chart legend and to set the number of
     * previous time-steps of data to display in the chart.
     *
     * @param title         Title of the chart
     * @param xaxis         Name of the x-axis
     * @param yaxis         Name of the y-axis
     ...
     *                      {@code maxSamples} updates will be displayed in the chart, so if the chart is updated at
     *                      each {@code time-step}, then only the most recent {@code maxSamples} time-steps will be
     *                      shown on the chart. If the user wishes to accumulate all data points from the simulation
     ...
     *
     * @param legend            The legend name of the series.
     * @param plottableObject_X The data source object implementing the FloatSource
     *                          interface to produce values for the x-axis (domain).
     * @param plottableObject_Y The data source object implementing the FloatSource
    ...
    public void addSeries(String legend, IntSource plottableObject_X,
                          Enum<?> variableID_X, IntSource plottableObject_Y, Enum<?> variableID_Y) {
    ...
    public void addSeries(String legend, Object target_X, String variableName_X, boolean getFromMethod_X,
        ...
        if (ReflectionUtils.isDoubleSource(target_Y.getClass(), variableName_Y, getFromMethod_Y))
            sourceY = new DSource(legend, new DoubleInvoker(target_Y, variableName_Y, getFromMethod_Y),
                    DoubleSource.Variables.Default);
        else if (ReflectionUtils.isFloatSource(target_Y.getClass(), variableName_Y, getFromMethod_Y))
            sourceY = new FSource(legend, new FloatInvoker(target_Y, variableName_Y, getFromMethod_Y),
                    FloatSource.Variables.Default);
        else if (ReflectionUtils.isIntSource(target_Y.getClass(), variableName_Y, getFromMethod_Y))
            sourceY = new ISource(legend, new IntegerInvoker(target_Y, variableName_Y, getFromMethod_Y),
                    IntSource.Variables.Default);
    ...
    /**
     * Build a series of paired values, retrieving x-axis data from a LongSource object and y-axis data from a
    ...
    public void addSeries(String legend, LongSource plottableObject_X,
                          Enum<?> variableID_X, DoubleSource plottableObject_Y, Enum<?> variableID_Y) {
        LSource sourceX = new LSource(legend, plottableObject_X, variableID_X);
        DSource sourceY = new DSource(legend, plottableObject_Y, variableID_Y);
        sources.add(new Pair<>(sourceX, sourceY));
        XYSeries series = new XYSeries(legend);
    ...
    public void refresh() {
        for (Object datum : dataset.getSeries()) {
            XYSeries series = (XYSeries) datum;
            series.clear();
        }
    }
